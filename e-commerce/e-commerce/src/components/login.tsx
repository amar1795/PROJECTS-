...
const Login:React.FC<LoginProps> = ({toggleView}) => {
  ...
  const { register: registerField, handleSubmit, formState: { errors }, reset   }= useForm<z.infer<typeof LoginSchema>>({
  ...
  return (
    <div>
      <div className=" mt-8">
            ...
            {error && <span className=" italic text-red-950  text-[1.5rem]" >{error}</span>}
            <form className="mt-8" onSubmit={handleSubmit(onSubmit)} >
              <div className="flex flex-col items-center">
                ...
                <div className=" h-[4rem]">
                  <button type='submit' className="w-80  p-2 border-2 border-black text-black mt-4 flex self-center justify-center border-b-8 border-r-4 active:border-b-2 active:border-r-2 bg-yellow-500">
